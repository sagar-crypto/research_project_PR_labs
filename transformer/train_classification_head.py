...
from sklearn.metrics import accuracy_score, precision_recall_fscore_support, roc_auc_score, f1_score, precision_recall_curve, average_precision_score
...
CLS_EPOCHS = 20                  # total classification epochs
FREEZE_EPOCHS = 4               # keep backbone frozen for first N epochs
HEAD_DROPOUT = 0.3              # a bit more regularization in the head
UNFREEZE_LAYERS = ("layers.3", "layers.2")  # encoder layers to unfreeze later
...
def count_labels(subset):
    ...
    for i in range(len(subset)):
        _, y = subset[i]
        yv = float(y if not torch.is_tensor(y) else y.item())
        pos += int(yv > 0.5); tot += 1
    return pos, tot

def collate_classify(batch):
    xs, ys = zip(*batch)
    xs = torch.stack([x.contiguous().to(torch.float32) for x in xs], 0)   # (B,L,D) fp32
    ys = torch.stack([ y if torch.is_tensor(y) else torch.tensor(y, dtype=torch.float32) ], 0) \
        if isinstance(ys, torch.Tensor) else torch.stack(
...
def make_weighted_sampler(subset):
    ...
    labels_t = torch.tensor(labels, dtype=torch.long)  # 0/1
    counts = torch.bincount(labels_t, minlength=2).float()   # [neg, pos]
    class_w = (1.0 / counts.clamp_min(1.0))                 # inverse freq
    weights = class_w[labels_t]                             # per-sample weights (CPU tensor)

    # Use list to satisfy static type checkers
...
def metrics_at_threshold(probs, y_np, thr):
...
def tune_threshold(probs: np.ndarray, y_np: np.ndarray) -> Tuple[float, float]:
    ...
    f1s = np.array([
        ...
        for t in ths
    ], dtype=float)  # now it's a float ndarray
    i = int(f1s.argmax())
    return float(ths[i]), float(f1s[i])

def compute_pos_weight(loader, device, max_batches=200):
    ...
    pos = max(1, pos); neg = max(1, tot - pos)
...
def confusion_from_probs(probs: np.ndarray, y: np.ndarray, thr: float):
    ...
    # precision/recall/f1 from counts (avoids any sklearn averaging gotchas)
    prec = tp / max(1, tp + fp)
    rec  = tp / max(1, tp + fn)
...
def main():
    ...
    events_map = make_event_map(LABELS_CSV) # dict: "replica_123" -> (start_s, end_s)
   ...
   # --- split ---
    ...
    # --- sampler: balance batches; DO NOT also use pos_weight ---
    ...
    clf = FaultClassifier(backbone, dropout=HEAD_DROPOUT, pool="last").to(device)
    ...
    epochs_cls = cfg.get("cls_epochs", 5)
    criterion = nn.BCEWithLogitsLoss()
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, clf.parameters()), lr=1e-3, weight_decay=1e-4)


    best = {"f1": -1.0, "state": None, "thr": 0.5}
    epochs_cls = cfg.get("cls_epochs", CLS_EPOCHS)
    ...
    for epoch in range(1, epochs_cls + 1):
        ...
        if epoch == (FREEZE_EPOCHS + 1):
            for name, p in clf.backbone.encoder.named_parameters():
                if any(tag in name for tag in UNFREEZE_LAYERS):
                    p.requires_grad = True
            head_params, backbone_params = [], []
            for n, p in clf.named_parameters():
                if not p.requires_grad:
                    continue
                (backbone_params if "backbone.encoder" in n else head_params).append(p)
            ...
            print(f"🔓 Unfroze {UNFREEZE_LAYERS} with two-group LRs (head 1e-3, enc 3e-4)")
        ...
        y_np      = val_y.cpu().numpy().astype(int)            # define BEFORE prints
        ...
        print("val pos rate:", y_np.mean(),
            " frac>=0.5:", float((val_probs >= 0.5).mean()))
        ...
        # choose operating threshold on VAL (max F1 on PR curve)
        ...
        print(f"[{epoch:02d}] val loss {val_loss:.4f} | AUC {val_auc:.3f} | "
            f"@τ={best_thr:.3f} → acc {accT:.3f} P {pT:.3f} R {rT:.3f} F1 {f1T:.3f}")

        # track best by VAL F1
        if f1T > best["f1"]:
            best["f1"]   = f1T
            best["thr"]  = float(best_thr)
            # save small CPU copy to avoid GPU memory bloat
    ...
    if best["state"] is not None:
        ...
        # primary operating point = VAL-picked threshold
        ...
        # Optional: if the val threshold yields 0 predicted positives on test,
        # ALSO report a non-leaky quantile fallback (no labels used to choose it)
        ...
        # PR curve and raw outputs (nice to inspect)
        ...
        if best["state"] is not None:
            ...
            print(f"Saved best classifier head to {out_path} (best val F1={best['f1']:.3f}, thr={best['thr']:.2f})")
        else:
            print("No improvement recorded; nothing saved.")
